...
namespace Client1
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // Client kendi RSA anahtar çiftini oluşturur.
            ...
            // Sertifikaları doğrulamak için CA'nın açık anahtarına ihtiyacımız var.
            string caPublicKey = GetCAPublicKey();
            Console.WriteLine("Got CA Public Key.");

            // 3. Sertifika İste (Request Certificate)
            // Kendi Public Key'imizi CA'ya gönderip imzalı sertifika alıyoruz.
            ...
            // 4. Client 2'yi Bekle (Listen for Client 2)
            // Client 1, bu senaryoda sunucu gibi davranıp Client 2'nin bağlanmasını bekler.
            int port = 6001;
            TcpListener listener = new TcpListener(System.Net.IPAddress.Any, port);
            listener.Start();
            Console.WriteLine($"Client 1 listening for Client 2 on port {port}...");
            ...
            // Kendi sertifikamızı gönderiyoruz.
            ...
            Console.WriteLine("Sent Certificate to Client 2.");

            // Client 2'nin sertifikasını alıyoruz.
            ...
            Console.WriteLine($"Received Certificate from {client2Cert.SubjectID}");

            // 6. Sertifika Doğrulama (Verify Client 2 Cert)
            // Gelen sertifikanın gerçekten CA tarafından imzalanıp imzalanmadığını kontrol ediyoruz.
            if (!CryptoHelper.VerifyData(client2Cert.GetDataToSign(), client2Cert.Signature!, caPublicKey))
            {
                Console.WriteLine("Client 2 Certificate Verification FAILED!");
                return;
            }
            Console.WriteLine("Client 2 Certificate Verified.");
            ...
            // Payload Hazırla: ID|Nonce1
            string payload1 = $"Client1|{nonce1Str}";
            byte[] payload1Bytes = Encoding.UTF8.GetBytes(payload1);

            // Payload'ı Client 2'nin Public Key'i ile Şifrele (Sadece o okuyabilsin)
            ...
            Console.WriteLine("Sent Encrypted ID and Nonce1 (N1).");

            // Client 2'den Şifreli Payload Al
            stream.ReadExactly(lenBuf, 0, 4);
            len = BitConverter.ToInt32(lenBuf, 0);
            byte[] encryptedPayload2 = new byte[len];
            stream.ReadExactly(encryptedPayload2, 0, len);

            // Payload'ı Kendi Private Key'imizle Çöz
            ...
            Console.WriteLine($"Received Encrypted Payload: {decryptedPayload2}");

            // ID ve Nonce2'yi Ayrıştır
            string[] parts = decryptedPayload2.Split('|');
            if (parts.Length != 2 || parts[0] != "Client2")
            {
                Console.WriteLine("Invalid Payload from Client 2!");
                return;
            }
            ...
            // Nonce3'ü Master Key ile Şifrele (AES)
            ...
            Console.WriteLine("Sent Encrypted Nonce3 (N3) with Master Key.");

            // Client 2'den Şifreli Nonce4 Al
            stream.ReadExactly(lenBuf, 0, 4);
            len = BitConverter.ToInt32(lenBuf, 0);
            ...
            Console.WriteLine("Received and Decrypted Nonce4 (N4).");
            ...
            // Mesaj Gönder (Session Key ile Şifreli)
            string message = "Hello Client 2, this is a secure message from Client 1!";
            byte[] iv = new byte[16]; // Her mesaj için rastgele IV kullanılmalı
            ...
            Console.WriteLine($"Sent Encrypted Message: {message}");
        ...
        static Certificate RequestCertificate(string clientId, string publicKey)
        {
            ...
            byte[] buffer = new byte[8192]; // Bigger buffer for cert
            int bytesRead = stream.Read(buffer, 0, buffer.Length);
